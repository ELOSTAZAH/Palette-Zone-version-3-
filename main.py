...
class DrawingApp:
        ...
        if vegetable == "Carrot":
            ...
            for i in range(5):
                ...
                pygame.draw.line(surface, (150, 150, 150) if not colored else (100, 180, 100), 
                               (start_x, start_y), (int(end_x), int(end_y)), 8)
    ...
    def draw_flower(self, surface, x, y, size):
        ...
        for i in range(6):
            ...
            pygame.draw.circle(surface, (255, 150, 150), (int(petal_x), int(petal_y)), size // 2)
        
        # Draw center
        pygame.draw.circle(surface, (255, 255, 100), (x, y), size // 3)
    
    def draw_bird(self, surface, x, y, size):
        """Helper function to draw a bird"""
        # Draw body
        pygame.draw.ellipse(surface, (100, 100, 100), (x - size, y - size//2, size*2, size))
        # Draw wing
        pygame.draw.ellipse(surface, (80, 80, 80), (x - size//2, y - size//3, size, size//2))
        # Draw beak
        pygame.draw.polygon(surface, (255, 200, 0), [(x + size, y), (x + size + size//2, y - size//4), (x + size, y + size//4)])
    ...
    def draw_difference_images(self):
        ...
        for i in range(8):
            ...
            pygame.draw.line(self.drawing_surface, (255, 240, 100), 
                           (int(start_x), int(start_y)), (int(end_x), int(end_y)), 4)
    ...
    def draw_reward(self):
        ...
        if self.reward_type == "star":
            ...
            for i in range(5):
                ...
                points.append((
                    center_x + size * 0.5 * math.cos(angle + math.pi/5),
                    center_y + size * 0.5 * math.sin(angle + math.pi/5))
                )
            pygame.draw.polygon(screen, (255, 255, 100), points)
        
        elif self.reward_type == "flower":
            # Draw a flower
            self.draw_flower(screen, center_x, center_y, int(size))
        
        elif self.reward_type == "clap":
            # Draw clapping hands
            text = self.title_font.render("Great Job!", True, (255, 100, 100))
            ...
            text = self.title_font.render("Try Again!", True, (255, 100, 100))
            screen.blit(text, (center_x - text.get_width()//2, center_y - text.get_height()//2))
    ...
    def draw_ui(self):
        ...
        # Draw title
        title = self.title_font.render("Palette Zone for Autistic", True, TEXT_COLOR)
        screen.blit(title, (SCREEN_WIDTH//2 - title.get_width()//2, 10))
        
        # Draw book selection buttons
        for button in self.buttons:
            if button["action"] == "select_book":
                color = HIGHLIGHT_COLOR if button["book_index"] == self.current_book_index else BUTTON_BG
        ...
        # Draw page info
        page_info = f"Page {self.current_page}/{self.total_pages} - {self.page_type.title()}"
        page_text = self.small_font.render(page_info, True, TEXT_COLOR)
        screen.blit(page_text, (50, 80))
        
        # Draw color palette
        if self.page_type == "coloring":
            palette_title = self.small_font.render("Colors:", True, TEXT_COLOR)
            screen.blit(palette_title, (50, 70))
            
            for button in self.color_buttons:
                pygame.draw.rect(screen, button["color"], button["rect"])
                if button["color"] == self.current_color:
                    pygame.draw.rect(screen, TEXT_COLOR, button["rect"], 3)
            
            # Draw brush sizes
            brush_title = self.small_font.render("Brush Size:", True, TEXT_COLOR)
            screen.blit(brush_title, (200, 70))
            
            for button in self.brush_buttons:
                color = HIGHLIGHT_COLOR if button["size"] == self.brush_size else BUTTON_BG
                pygame.draw.rect(screen, color, button["rect"])
                pygame.draw.rect(screen, TEXT_COLOR, button["rect"], 2)
                
                size_text = self.small_font.render(str(button["size"]), True, TEXT_COLOR)
                screen.blit(size_text, (button["rect"].centerx - size_text.get_width()//2,
                                      button["rect"].centery - size_text.get_height()//2))
        
        # Draw surfaces
        # Reference surface (left side)
        ref_rect = pygame.Rect(10, 150, SCREEN_WIDTH//2 - 20, SCREEN_HEIGHT - 250)
        pygame.draw.rect(screen, (255, 255, 255), ref_rect)
        pygame.draw.rect(screen, TEXT_COLOR, ref_rect, 2)
        screen.blit(self.reference_surface, (10, 150))
        
        ref_title = self.small_font.render("Reference", True, TEXT_COLOR)
        screen.blit(ref_title, (10, 125))
        
        # Drawing surface (right side)
        draw_rect = pygame.Rect(SCREEN_WIDTH//2 + 10, 150, SCREEN_WIDTH//2 - 20, SCREEN_HEIGHT - 250)
        pygame.draw.rect(screen, (255, 255, 255), draw_rect)
        pygame.draw.rect(screen, TEXT_COLOR, draw_rect, 2)
        screen.blit(self.drawing_surface, (SCREEN_WIDTH//2 + 10, 150))
        
        if self.page_type == "coloring":
            draw_title = self.small_font.render("Your Drawing", True, TEXT_COLOR)
        elif self.page_type == "differences":
            draw_title = self.small_font.render(f"Find Differences ({len(self.found_differences)}/15)", True, TEXT_COLOR)
        else:
            draw_title = self.small_font.render("Word Similarity", True, TEXT_COLOR)
        
        screen.blit(draw_title, (SCREEN_WIDTH//2 + 10, 125))
        
        # Draw navigation buttons
        for button in self.buttons:
            if button["action"] in ["prev_page", "next_page", "check_work"]:
                pygame.draw.rect(screen, BUTTON_BG, button["rect"], border_radius=10)
                pygame.draw.rect(screen, TEXT_COLOR, button["rect"], 2, border_radius=10)
        ...
        # Draw license info
        license_text = self.small_font.render(f"© {self.license_info['owner']} - {self.license_info['license']}", True, TEXT_COLOR)
        screen.blit(license_text, (10, SCREEN_HEIGHT - 20))
        
        # Draw download counter
        download_text = self.small_font.render(f"Downloads: {self.downloads}", True, TEXT_COLOR)
        screen.blit(download_text, (SCREEN_WIDTH - 150, 10))
